...
mutual
  ...
  -- A handler for effect ℓ: a σ'-indexed clause for every operation of ℓ
  -- (HANDLER rule), plus a return clause. Parameter-free: unlike the
  -- original λC, a handler carries no extra state/parameter value
  -- threaded alongside the operation clauses -- each clause simply
  -- receives the operation's own argument, a choice continuation, and a
  -- delimited (resume) continuation, each unary (not paired with a
  -- parameter).
  --
  ...
  record Handler (Γ : Cxt) (ℓ : Effect) (σ σ' : Ty) (ε : EffCxt) : Set where
    inductive
    field
      clause : (op : Op ℓ)
             → ((Γ , gnd (out op)) , (gnd (in′ op) ⇒ Loss ! ε))
                 , (gnd (in′ op) ⇒ σ' ! ε)
               ⊢ σ' ! ε
      ret    : Γ , σ ⊢ σ' ! ε
  ...
  data _⊢_!_ (Γ : Cxt) : Ty → EffCxt → Set where
    ...
    opE     : ∀ {ℓ ε σ}     → ℓ ∈ ε → (op : Op ℓ) → σ ≡ gnd (in′ op) → Γ ⊢ gnd (out op) ! ε → Γ ⊢ σ ! ε
    ...
    handleE : ∀ {ℓ σ σ' ε} → Handler Γ ℓ σ σ' ε → Γ ⊢ σ ! (ε ,ℓ ℓ) → Γ ⊢ σ' ! ε
