...
pgm = opE m∈εNdet decide refl (val (vgnd tt))

-- A handler for ndet that always resumes with `true`, discarding the
-- choice continuation (a trivial, non-choosing handler, just to exercise
-- the construction). Parameter-free: no extra state threaded alongside
-- the operation clauses.
hAlways : Handler ∅ ndet (gnd (base bool)) (gnd (base bool)) []
-- z : (out, choiceCont, delimCont) -- Z is delimCont (innermost).
clause hAlways decide = app (val (vvar Z)) (val (vgnd true))
ret    hAlways        = val (vvar Z)

-- with hAlways handle pgm : Bool ! []
handled : ∅ ⊢ gnd (base bool) ! []
handled = handleE hAlways pgm
...
handledValueStep : (g : LC ∅ (gnd (base bool)) []) → g ⊢ handleE hAlways (val (vgnd false)) -[ 0 ]→ val (vgnd false)
handledValueStep g = R6 hAlways (vgnd false)
