...
mutual
  ...
  renH : ∀ {Γ Γ' ℓ σ σ' ε} → Ren Γ Γ' → Handler Γ ℓ σ σ' ε → Handler Γ' ℓ σ σ' ε
  clause (renH ρ h) op = renE (extR (extR (extR ρ))) (clause h op)
  ret    (renH ρ h)    = renE (extR ρ) (ret h)
  ...
  renE ρ (opE m op σeq e) = opE m op σeq (renE ρ e)
  ...
  renE ρ (handleE h e)    = handleE (renH ρ h) (renE ρ e)
...
mutual
  ...
  subH : ∀ {Γ Γ' ℓ σ σ' ε} → Sub Γ Γ' → Handler Γ ℓ σ σ' ε → Handler Γ' ℓ σ σ' ε
  clause (subH σs h) op = subE (extS (extS (extS σs))) (clause h op)
  ret    (subH σs h)    = subE (extS σs) (ret h)
  ...
  subE σs (opE m op σeq e) = opE m op σeq (subE σs e)
  ...
  subE σs (handleE h e)    = handleE (subH σs h) (subE σs e)
